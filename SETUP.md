...
F: NimBLE init ok
G: BLE power set
G2: coexistence set to prefer BT
H: WiFi started
I: setup complete, heap 210000
[WiFi] up, IP 192.168.x.x
[BLE] connecting...
[BLE] connected
[BLE] ffe1 handle=0x05 notify=1 writeNR=1
[BLE] subscribed to 0xffe1
[BLE] wake sequence sent
[web] starting (BMS link up), heap 148000
[web] http://solarpulse.local/
```

**The order matters.** `[BLE] connected` comes *before* `[web] starting`. WiFi and BLE share one 2.4 GHz radio, and the web server is deliberately held back until the BMS link is up so NimBLE gets a quiet radio and a full heap to connect with. If the BMS never appears, the server starts anyway after 90 s so you can still reach the page to debug.

**Check:** you see `[BLE] subscribed to 0xffe1` and no repeating `connect failed` lines, and `http://solarpulse.local/` (or the IP) opens the local dashboard in a browser on the same network.
...
| BLE worked, then stopped after adding WiFi features | `WiFi.setSleep(false)` somewhere | **Never call it.** WiFi and BLE share one radio; disabling modem sleep makes WiFi hold it continuously and starves BLE. Leave the default, and keep `esp_coex_preference_set(ESP_COEX_PREFER_BT)` |
| BMS connects but no frames arrive | Radio contention, or TLS during connect | Confirm `G2: coexistence set to prefer BT` appears at boot. Cloud pushes are already held off until the link is up |
| `[BLE] connected` never appears, `[web] starting` does | Heap exhausted before NimBLE could connect | Check `heap=` in the `[stat]` line. Below ~80 kB, drop the local web server and use the Firebase dashboard only |
