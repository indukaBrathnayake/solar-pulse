...
## Radio coexistence — read this before touching startup

The ESP32 has **one** 2.4 GHz radio. WiFi and BLE take turns on it, arbitrated
by the coexistence driver. The BMS link is a *connection*: miss enough slots and
it drops, and reconnecting costs a full discovery round. A Firebase push is a
*transaction*: if it fails, it retries a second later and nothing is lost.

So the firmware is deliberately biased toward Bluetooth, in three places:

1. **`esp_coex_preference_set(ESP_COEX_PREFER_BT)`** in `setup()`, right after
   NimBLE comes up.
2. **WiFi modem sleep stays ON.** Do **not** call `WiFi.setSleep(false)`. It
   feels like a free latency win for the web server, but it makes the WiFi
   stack hold the radio continuously and the BMS then either never connects or
   connects and delivers no frames. This exact line cost a full debugging
   session — the comment in `setup()` says so.
3. **Startup order.** BLE initialises before WiFi, and the web server does not
   start in `setup()` at all — `webServerTask()` brings it up from `loop()`
   once the first BMS frame has arrived, or after 90 s if the BMS never shows.
   AsyncTCP's task and buffers are a large heap allocation, and NimBLE needs
   contiguous heap plus radio time to build the connection and walk the GATT
   table. Cloud pushes are held off during that same window for the same
   reason.

If you add anything that touches WiFi, TLS or RAM at boot, keep it behind the
same gate. `[BLE] connected` must appear *before* `[web] starting` in the log.
...
- Below the evening floor with no sun, it moves to utility. Which floor
  applies depends on the rainy-day check below.
- It only goes back to the pack once SoC has recovered by `SOC_RECOVER_HYST`
  (5 points) above whichever floor is active, so a sagging pack cannot
  chatter the relays.

**Rainy-day check, once per day at 18:15**

At `RAINY_CHECK_HOUR:RAINY_CHECK_MIN` (18:15) the firmware looks at whether
today's SoC ever reached `SOC_TARGET_1600` (99%):

- **Reached it** — plenty of sun today, keep the normal `SOC_EVENING_FLOOR`
  (60%) for the rest of the night.
- **Missed it** — most likely a cloudy or rainy day. Forcing the normal 60%
  floor here would mean falling back to utility on every such evening, which
  defeats the point of the battery. Instead the floor relaxes to
  `SOC_RAINY_FLOOR` (35%) for the rest of tonight: the house runs off the
  pack down to 35% before utility is used at all, and utility switches off
  again once the pack recovers past 35% + `SOC_RECOVER_HYST`.

This runs once and locks in for the night; it resets at the next midnight
rollover. The result (`rainyModeTonight`) and both floors are visible at
`/api/config` on the device.
...
It starts once the BMS link is up (see the coexistence note above), so expect
it to be unreachable for the first few seconds after a reset.
